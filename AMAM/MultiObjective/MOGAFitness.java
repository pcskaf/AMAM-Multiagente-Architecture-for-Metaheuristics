...
public class MOGAFitness extends MultiObjectiveFitness{
	...
	//alpha - define se escalonamento � linear (alpha = 1) ou n�o (alpha > 1) 
	public void evaluateFitnessMOGA(Population pop, int alpha, int size_file, Problem p) {
		
		double fitness[] = new double[pop.getSize()];
		
		/*- Classifica��o -*/
		//determina a classifica��o de cada indiv�duo da popula��o
		pop.rankIndividuals(p);
		
		/*- Normaliza��o -*/
		//normaliza os valores de r(i) para o intervalo unit�rio e transforma para maximiza��o
		...
		//escalonamento, se alpha = 1 - escalonamento linear -- AT� AQUI --
		
		//caso o escalonamento n�o seja linear (alpha > 1), normaliza os valores novamente
		...
		//A fun��o de aptid�o de cada indiv�duo � degradada com base na fun��o de partilha
		...
		//Calcula sigma_sh - raiz de m (maior diagonal do hipercubo) dividido por X (tamanho m�ximo do arquivo)
		double sigma_sh;
	...
	public Population updateFileMOGA(Population pop, Population file, int size_file_max, int receiver_size, Problem p) {
	    //une a popula��o corrente com o arquivo j� existente para idenficar 
		//os pontos de pareto da popula��o corrente
		...
		//identifica o pareto da popula��o corrente
		currente_file = currente_file.identifyPareto(receiver_size, p);
		
		//verifica o tamanho do arquivo para poss�vel redu��o
		if(currente_file.getSize() > size_file_max) {
			//reduz o arquivo ao tamanho m�ximo
	...
	public Population reduceFileMOGA(Population file, int size_file_max, int receiver_size, Problem p) {
		...
		while (current_size > size_file_max) {
			...
			//Elimina o indiv�duo com pior c
